...
type
  ...
  private
    hMutex: hwnd;
      //mutex so we wouldn't access memory-mapped mIRC file when something is using it. Do not change mutex name!
    FhFileMap: THandle;
    FmData: pchar;
  end;
...
begin
  inherited Create;
  if mHandle = 0 then
    mHandle := GetMircHandle;
  mircHandle := mHandle;
  FhFileMap := CreateFileMapping(INVALID_HANDLE_VALUE, nil, PAGE_READWRITE, 0,
    4096, 'mIRC');
...
begin
  if hmutex <> 0 then
    WaitForSingleObject(hMutex, 1000);
      //wait 1 sec. if mutex is not released in 1 sec then mIRC is f*cked anyway
  StrPCopy(FmData, c);
...
function TmIRCManager.ReadInteger(const Ident: string; const Default: integer):
  integer;
begin
...
function TmIRCManager.ReadFloat(const Ident: string; const Default: Double):
  Double;
var
  FloatStr: string;
begin
...
function TmIRCManager.ReadBool(const Ident: string; const Default: boolean):
  boolean;
var
  BoolStr: string;
begin
...
//on & off are for $group().status
  if (BoolStr = '1') or (BoolStr = '$true') or (BoolStr = '-1') or (BoolStr =
    'on') then
    result := true;
  if (BoolStr = '0') or (BoolStr = '$false') or (BoolStr = 'off') then
    result := false;
end;
...
begin
  if Value then
    command('//set ' + ident + ' $true')
  else
    command('//set ' + ident + ' $false');
end;
